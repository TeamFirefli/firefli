...
export async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const workspaceGroupId = config.workspaceGroupId;
    ...
    if (current === "true") {
      ...
      where.action = { in: ["perm_ban", "temp_ban"] };
      where.OR = [{ isPermanent: true }, { expiresAt: { gt: now } }];
    }
    ...
    const filteredCases =
      current === "true"
        ? cases.filter((c) => {
            ...
            if (!["temp_ban", "perm_ban"].includes(c.action || "")) return false;

            return activeBans.some((ban) => {
              if (ban.userId !== c.targetUserId) return false;
              if (c.action === "perm_ban") return ban.expiresAt === null;
              if (c.action === "temp_ban") return !!ban.expiresAt && ban.expiresAt > now;
              return false;
            });
          })
        : cases;
    ...
    console.error("Error fetching moderation case history:", error);
    return res
      .status(500)
